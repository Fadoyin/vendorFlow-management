...
import { vendorApi, ordersApi } from '@/lib/api'
...
export default function VendorOrdersContent() {
  ...
  const handleViewOrder = async (order: Order) => {
    try {
      setActionLoading(`view-${order.id}`);
      
      // Try vendor-specific endpoint first, fallback to general orders API
      let response;
      try {
        response = await vendorApi.getOrderById(order._id || order.id);
      } catch (vendorError) {
        console.log('Vendor-specific endpoint failed, trying general orders API:', vendorError);
        response = await ordersApi.getById(order._id || order.id);
      }
      
      if (response.data) {
        setSelectedOrder({ ...order, ...response.data });
        setShowViewModal(true);
      } else {
        // If API fails, show modal with existing order data
        setSelectedOrder(order);
        setShowViewModal(true);
      }
    } catch (err) {
      console.error('Error fetching order details:', err);
      // Show modal with existing data even if API fails
      setSelectedOrder(order);
      setShowViewModal(true);
    } finally {
  ...
  const handleCancelOrder = async (order: Order) => {
    ...
    try {
      ...
      // Try vendor-specific endpoint first, fallback to general orders API
      try {
        await vendorApi.updateOrderStatus(order._id || order.id, 'cancelled');
      } catch (vendorError) {
        console.log('Vendor-specific endpoint failed, trying general orders API:', vendorError);
        await ordersApi.updateStatus(order._id || order.id, 'cancelled');
      }
      ...
      alert('Failed to cancel order. This may be due to insufficient permissions or the order may not be cancellable.');
    } finally {
      setActionLoading(null);
    }
  };

  const handleEditSuccess = async (updatedOrder: any) => {
    try {
      // Try vendor-specific endpoint first, fallback to general orders API
      try {
        await vendorApi.updateOrder(selectedOrder!._id || selectedOrder!.id, updatedOrder);
      } catch (vendorError) {
        console.log('Vendor-specific endpoint failed, trying general orders API:', vendorError);
        await ordersApi.update(selectedOrder!._id || selectedOrder!.id, updatedOrder);
      }
      ...
      alert('Failed to update order. This may be due to insufficient permissions.');
    }
  };
    ...
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="mb-6">
          ...
          <p className="text-gray-600">Manage your orders and track fulfillment</p>
        </div>
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Orders</h1>
        <p className="text-gray-600">Track and manage all your orders from customers</p>
      </div>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {ordersData.orders.map((order) => (
                          <tr key={order.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm font-medium text-gray-900">{order.supplierName || 'N/A'}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                              ...
                              >
                                {actionLoading === `view-${order.id}` ? 'Loading...' : 'View'}
                              </button>
                              <button 
                                onClick={() => handleEditOrder(order)}
                                disabled={order.status === 'cancelled' || order.status === 'completed'}
                                className="text-green-600 hover:text-green-900 mr-3 disabled:opacity-50 disabled:text-gray-400"
                              >
                                Edit
                              </button>
                              <button 
                                onClick={() => handleCancelOrder(order)}
                                disabled={actionLoading === `cancel-${order.id}` || order.status === 'cancelled' || order.status === 'completed'}
