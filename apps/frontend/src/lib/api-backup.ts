...
// Add OTP methods
(ApiService.prototype as any).sendOtp = async function(email: string, purpose: 'signup' | 'login') {
  return this.request('auth/send-otp', {
    method: 'POST',
    body: JSON.stringify({ email, purpose }),
  });
...
(ApiService.prototype as any).verifyOtp = async function(email: string, otp: string, purpose: 'signup' | 'login') {
  const response = await this.request('auth/verify-otp', {
    method: 'POST',
    body: JSON.stringify({ email, otp, purpose }),
  });
